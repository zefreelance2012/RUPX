...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != heightOfBlockLastAccumulated) {
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("Rupaya Core Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("Rupaya Core Miner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("Rupaya Core Miner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Rupaya Core Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
