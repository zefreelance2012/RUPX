...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRupxAmount"))
        settings.setValue("nAnonymizeRupxAmount", 1000);

    nAnonymizeRupxAmount = settings.value("nAnonymizeRupxAmount").toLongLong();
...
// Wallet
    ...
    if (!settings.contains("addrProxy"))
        settings.setValue("addrProxy", "127.0.0.1:9025");
    ...
    if (settings.contains("nAnonymizeRupxAmount"))
        SoftSetArg("-anonymizerupxamount", settings.value("nAnonymizeRupxAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRupxAmount:
            return QVariant(nAnonymizeRupxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRupxAmount:
            nAnonymizeRupxAmount = value.toInt();
            settings.setValue("nAnonymizeRupxAmount", nAnonymizeRupxAmount);
            emit anonymizeRupxAmountChanged(nAnonymizeRupxAmount);
            break;
        case CoinControlFeatures:
