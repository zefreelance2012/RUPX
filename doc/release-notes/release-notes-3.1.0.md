...
RUPX/ZRupx Staking and Masternode Rewards
--------------

### RUPX, ZRupx and Masternode Payment Schedule
...
- 3 ZRupx (3 x 1 denominations) rewarded to the staker, 2 RUPX rewarded to the masternode owner and 1 RUPX available for the budget. This is a total block reward of 6 RUPX, up from 5.

If a user staking RUPX wins the reward, the following amounts will be rewarded: 
- 2 RUPX to the RUPX staker, 3 RUPX to the Masternode owner and 1 RUPX available for the budget. This is a total block reward of 6 RUPX, up from 5.


### Return change to sender when minting ZRupx

Previously, ZRupx minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most RUPX and return the change from a ZRupx mint to that address.
...
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular RUPX.
...
As voted on by the Rupaya masternodes, the monthly budget available to be utilised has been increased to 42,000 RUPX / month. This RUPX only has the opportunity to be raised once per month (paid to winning proposals) with any unused RUPX not created by the blockchain.

### Budget Finalization Fee

The Rupaya finalization fee for successful proposals has now been reduced, this fee is now 5 RUPX down from 50 RUPX. The total fee outlay for a successful proposal is now a total of 55 RUPX.
...
Zerocoins, or ZRupx, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of ZRupx that can be recalculated at any time using the seed. Deterministic ZRupx allows for users to backup all of their future ZRupx by simply recording their seed and keeping it in a safe place (similar to backing up a private key for RUPX). The ZRupx seed needs to remain in the wallet in order to spend the ZRupx after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private ZRupx data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
